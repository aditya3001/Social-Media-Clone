    ...
    return <li className="flex gap-4 border-b px-4 py-4">
        ...
        <div className="flex flex-grow flex-col">
            <div className="flex gap-1">
                <Link  href={`/profiles/${user.id}`} className="font-bold outline-none hover:underline 
...
function HeartButton({likedByMe, likeCount, isLoading, onClick} : HeartButtonProps){
    ...
    transition-color duration-200 ${likedByMe ? "text-red-500":"text-gray-500 focus-visible:text-red-500"}`}>
    <IconHoverEffect red>
        <HeartIcon className= {`transition-colors duration-200  ${likedByMe ? 
        "fill-red-500" :"group-hover:fill-red-500 fill-gray-500 group-focus-visible:fill-red-500"}`}/>
    </IconHoverEffect>
    <span>{likeCount}</span>
    </button>)
}